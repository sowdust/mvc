...
//	VARIABILI D'ISTANZA
    ...
    function __construct($db, $table, $parameters, $values, $start = 0, $offset = 20, $order_by = '', $reverse = false) {
        ...
        foreach ($parameters as $count => $p) {
            ...
            if (self::$tipi[$table][$p] == 's' && !regexp::testo($values[$count])) {
                die('5');
            }
            if (self::$tipi[$table][$p] == 'i' && (!is_numeric($values[$count]) && $values[$count] != '')) {
                die('6');
            }
        }
